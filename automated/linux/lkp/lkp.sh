...
report_atomic_job()
{
	...
	if [[ ! -e "${TEST_DIR}/working/result" ]] ; then
		...
		REPORT_CODE="1"
	else	# gather atomic_job exit status accordingly test-name
		case "${test}" in

		sysbench-cpu)
			# 'grep' return 1 if !found or 2 with error (file does not exists)
			# 'exits_fail' appears in the output only if sysbench-cpu test failed
			grep -r "exit_fail" "${TEST_DIR}/working/result" >/dev/null
			...
			;;
		*)
			error_msg "--${test} is unknown test, it is not possible to retrive exit_status"
			REPORT_CODE='unknown'
			;;
		esac
	fi

	# call the correct report function according with REPORT_CODE
	case "${REPORT_CODE}" in
	0)
		report_pass "${1}"
		;;
	...
	unknown)
		...
		;;
	*)
		error_msg "report_atomic_job: not defined REPORT_CODE"
		report_skip "${1}"
		;;
	esac
