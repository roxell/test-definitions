...
usage() {
    echo "\
    Usage: [sudo] ./mmtests.sh [-d <DURATION>] [-v <TEST_PROG_VERSION>]
    ...
    <TEST>:
    Benchmark test ro run.
    "
}

while getopts "h:p:u:s:v:t:" opt; do
    case $opt in
        u)
            if [[ "$OPTARG" != '' ]]; then
                TEST_GIT_URL="${OPTARG}"
...
install() {
    ...
    case "${dist}" in
        debian|ubuntu)
          ...
          pkgs="${pkgs} build-essential wget perl autoconf automake bc binutils-dev btrfs-progs linux-cpupower\
                expect gcc hdparm hwloc-nox libtool numactl tcl time xfsprogs xfslibs-dev libopenmpi-dev r-base"

          install_deps "${pkgs}" "${SKIP_INSTALL}"
          ;;
        fedora|centos)
          [[ -n "${TEST_GIT_URL}" ]] && pkgs="git-core"
          pkgs="${pkgs} gcc gcc-c++ kernel-devel make wget perl autoconf automake bc binutils-dev btrfs-progs linux-cpupower\
                expect gcc hdparm hwloc-nox libtool numactl tcl time xfsprogs xfslibs-dev libopenmpi-dev r-base"
          install_deps "${pkgs}" "${SKIP_INSTALL}"
          ;;
...
run_test() {
    pushd "${TEST_DIR}" || exit 1
    ./run-mmtests.sh --no-monitor --config "${TEST}" test-output
    popd || exit 1
}
