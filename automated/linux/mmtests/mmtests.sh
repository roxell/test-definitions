...
check_results() {
  ...
  if [ "$operations_check" != "true" ]; then
    echo "ERROR: _OperationsSeen is empty"
    return 1
  fi

  # Check if "_ResultData" is present in "results" and not empty
  result_data_check=$(jq '._ResultData | select(. != null) | keys | length > 0' "$1")
  if [ "$result_data_check" != "true" ]; then
    echo "ERROR: _ResultData is empty"
    return 1
  fi
...
collect_results() {
  ...
  for json in "${jsons[@]}"; do
    ...
    echo "INFO: results collected to ${OUTPUT}/$json"

    if [ "${FULL_ARCHIVE}" = "true" ]; then
      mv "${TEST_DIR}/work/log/${RESULTS_DIR}" "${OUTPUT}"
      echo "INFO: full archive is saved to ${OUTPUT}/${RESULTS_DIR}"
    fi
  done
